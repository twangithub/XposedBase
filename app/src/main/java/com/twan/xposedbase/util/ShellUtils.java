...
public class ShellUtils {
	...
	public static String getUiPid() {
		CommandResult res = execCommand("ps | grep uiautomator", false);
		String successMsg = res.successMsg;
		if (!TextUtils.isEmpty(successMsg)) {
			Pattern p = Pattern.compile("\\s+");
			Matcher m = p.matcher(successMsg);
			String pid = m.replaceAll(" ");
	...
	public static void unFrozenAllApp(){
		CommandResult res = execCommand("pm list packages -d", true);
		String successMsg = res.successMsg;
		if (!TextUtils.isEmpty(successMsg)) {
			if(successMsg.contains("package:")){
				successMsg = successMsg.substring(successMsg.indexOf("package:")+"package:".length());
				String[] packages = successMsg.split("package:");
				for(String pkgName:packages){
					unFrozenApp(pkgName);
				}
			}
		}else{
			res = execCommand("pm list packages -d", false);
	...
	public static void killUiPid(){
		execCommand("kill "+getUiPid(), true);
	}
